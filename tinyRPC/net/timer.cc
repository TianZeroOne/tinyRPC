...
void Timer::onTimer() {
    ...
    m_pending_events.erase(m_pending_events.begin(), it); // 删除重新加入的任务
    lock.unlock();
...
void Timer::resetArriveTime() {
    ...
    // INFOLOG("timp size=%d", tmp.size());
    if (!tmp.size()) { // 没有事件添加直接返回，作者这里有问题
        return;/*  */
    }
    int64_t now = getNowMs();
    auto it = tmp.begin();
...
void Timer::addTimerEvent(TimerEvent::s_ptr event) {
    ...
    lock.unlock();
    // INFOLOG("is_reset_timerfd=%d", is_reset_timerfd);
...
void Timer::deleteTimerEvent(TimerEvent::s_ptr event) {
    ...
    auto begin = m_pending_events.lower_bound(event->getArriveTime()); // 第一个匹配的key
    auto end = m_pending_events.upper_bound(event->getArriveTime());  // 最后一个匹配的key
